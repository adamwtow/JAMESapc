...
ros_kinfu_ls_publisher::ros_kinfu_ls_publisher(ros::NodeHandle nodehandle):nh_(nodehandle),it_(nodehandle)
{
    ...
    if (!nh_.getParam("depth_image_topic", depth_topic))depth_topic = "/ros_kinfu/depth/image";
...
void ros_kinfu_ls_publisher::updateKinectWorldTransform(tf::StampedTransform stamped_transform, Affine3f input_pose){
...
void ros_kinfu_ls_publisher::publishTSDFCloud(PointCloudTSDF::Ptr cloud_ptr){
...
void ros_kinfu_ls_publisher::publishCameraPose(Affine3f pose_in){
...
void ros_kinfu_ls_publisher::publishTFfromPose(Affine3f pose_in)
{
...
void ros_kinfu_ls_publisher::publishKfWorldTransform()
{
    kf_world_transform.stamp_ = ros::Time::now();
    kinfu_tf_broadcaster.sendTransform(kf_world_transform);
}

void ros_kinfu_ls_publisher::publishDepth(KinfuTracker::DepthMap depth)
{
    int c;
