...
{		
	...
	tf_listener_.waitForTransform("my_ws","camera_rgb_optical_frame",ros::Time(0), ros::Duration(10.0));
	tf_listener_.lookupTransform("my_ws","camera_rgb_optical_frame",ros::Time(0), locTfCamera2WsStemp_);
	...
	//std::cout << pts_cam_source << std::endl;
	*/
	PointCloud::Ptr cloud(new PointCloud);
	*cloud=*cloud_in;
...
std::vector<GraspHypothesis> Localization::filterHands(const std::vector<GraspHypothesis>& hand_list)
{
	...
	for (int i = 0; i < hand_list.size(); i++)
	{
		...
		geometry_msgs::PointStamped pt_centre, pt_centre_t;
		pt_centre.header.frame_id= "camera_rgb_optical_frame";
		pt_centre.point.x=center(0);
		pt_centre.point.y=center(1);
		pt_centre.point.z=center(2);
		tf_listener_.transformPoint("my_ws",pt_centre,pt_centre_t);
		Eigen::Vector3d center_t;
		center_t(0)=pt_centre_t.point.x;
		center_t(1)=pt_centre_t.point.y;
		center_t(2)=pt_centre_t.point.z;
		
		//std::cout<<"workspace_"<<workspace_<<std::endl;
		int k;
		for (k = 0; k < workspace_.size(); k++)
		{
			if (fabs((center_t(floor(k / 2.0)) - workspace_(k))) < MIN_DIST)
			{
				break;
			}
		}
