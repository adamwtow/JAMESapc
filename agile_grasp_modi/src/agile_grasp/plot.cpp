...
void Plot::createVisualPublishers(ros::NodeHandle& node, double marker_lifetime)
{
	...
	pickPose_pub_ = node.advertise<visualization_msgs::Marker>("pickPose_visual", 10);
...
void Plot::plotHandlesRviz(const std::vector<Handle>& handle_list, const std::string& frame)
{
	...
	if (handle_list.size()>0)
	{
		...
		tf::Transform trans;
		Eigen::Vector3d Pt0,Pt1,Pt_approach;
		Pt0=handle_list[0].getHandsCenter();
		Pt_approach=handle_list[0].getApproach();

		Pt1(0) = Pt0(0) - 0.07 * Pt_approach(0);
		Pt1(1) = Pt0(1) - 0.07 * Pt_approach(1);
		Pt1(2) = Pt0(2) - 0.07 * Pt_approach(2);
		...
		Eigen::Quaterniond q;
		Eigen::Vector3d axis_vector = Pt1 - Pt0;
		...
		pose.translation() = Pt1;
		tf::poseEigenToMsg(pose, output_vector);


/*
		Eigen::Quaterniond qz;//(1,0,0,0);
		Eigen::Affine3d k;
		//qz<<0,0,0,1;
		qz.setFromTwoVectors(Pt1, Pt0);
		//std::cout<<"qz="<<qz.w<<std::endl;
		qz.normalize();
		k.translate(Pt1);
		k.rotate(qz);
		transformEigenToTF(k,trans);
		tf::TransformBroadcaster br;
		br.sendTransform(tf::StampedTransform(trans, ros::Time::now(), "camera_rgb_optical_frame", "PickPose"));
...
*/
		...
		marker.color.b = 0;
		marker.color.a = 0.5;
		marker.pose.position.x = output_vector.position.x;//Pt1(0);
		marker.pose.position.y = output_vector.position.y;//Pt1(1);
		marker.pose.position.z = output_vector.position.z;//Pt1(2);
		marker.pose.orientation.x = q.x();
		marker.pose.orientation.y = q.y();
		marker.pose.orientation.z = q.z();
		marker.pose.orientation.w = q.w();
		pickPose_pub_.publish(marker);
	...
	const Eigen::Vector3d& approach, int id, const double* color, double alpha, double diam)
{
	...
	q.x = p.x - 0.07 * approach(0);
	q.y = p.y - 0.07 * approach(1);
	q.z = p.z - 0.07 * approach(2);
