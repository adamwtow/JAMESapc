...
std::string topic_image_rgb_reg_id = "/image/rgb_registered";
std::string topic_image_depth_raw_id = "/image/depth_raw";
std::string topic_image_infrared_raw_id = "/image/ir_raw";

//std::string topic_image_rgb_registered_id = "/image/rgb_raw";
//std::string topic_image_depth_registered_id = "/image/depth_raw";

std::string rgb_camera_info_url = "file://" + ros::package::getPath("realsense_camera_modified") + "/data/camera_info/realsense_rgb_camera.yaml";
std::string depth_camera_info_url = "file://" + ros::package::getPath("realsense_camera_modified") + "/data/camera_info/realsense_depth_camera.yaml";
...
image_transport::CameraPublisher realsense_rgb_registered_image_pub;
image_transport::CameraPublisher realsense_depth_image_pub;
image_transport::CameraPublisher realsense_depth_registered_image_pub;
camera_info_manager::CameraInfoManager *rgb_camera_info_man;
...
initDepthToRGBUVMap()
{
    ...
    std::string packagePath = ros::package::getPath("realsense_camera_modified");
...
pubRealSenseDepthImageMsg(cv::Mat& depth_mat)
{
    ...
    header.seq = head_sequence_id;
    header.stamp = head_time_stamp;
    header.frame_id = depth_frame_id;

    cv_bridge::CvImage* cv_msg_ptr = new cv_bridge::CvImage();
    cv_msg_ptr->header = header;
    cv_msg_ptr->image = depth_mat;
    cv_msg_ptr->encoding = sensor_msgs::image_encodings::TYPE_16UC1;

    depth_img = cv_msg_ptr->toImageMsg();

    //realsense_depth_image_pub.publish(depth_img);
    sensor_msgs::CameraInfo depth_info = depth_camera_info_man->getCameraInfo();
    realsense_depth_image_pub.publish(*depth_img,depth_info,depth_img->header.stamp);
}
...
pubRealSenseRGBImageMsg(cv::Mat& rgb_mat)
{
    ...
    sensor_msgs::CameraInfo rgb_info = rgb_camera_info_man->getCameraInfo();
    realsense_rgb_image_pub.publish(rgb_img,rgb_info,rgb_img.header.stamp);
}

void
pubRealSenseRGBRegImageMsg(cv::Mat& rgb_mat)
{
    sensor_msgs::Image rgb_img;
    ...
    sensor_msgs::CameraInfo rgb_info = rgb_camera_info_man->getCameraInfo();
    //realsense_rgb_registered_image_pub.publish(rgb_img,rgb_info,rgb_img.header.stamp);
}
...
processRGBD()
{
    ...
    cv::Mat rgb_frame;
    cv::Mat rgb_registered = cv::Mat_<cv::Vec3b>::zeros(depth_stream.height,depth_stream.width);
    ...
    for(int i=0; i<depth_stream.width * depth_stream.height; ++i)
    {
        ...
        if (depth){
            depth_frame_buffer[i] = (unsigned short)depth;
            //depth_frame_buffer[i] = 255 * (sensor_depth_max - depth) / sensor_depth_max;
        }else{
            depth_frame_buffer[i] = 0;
        }
        //depth_frame_buffer[i] = depth ? 65535 * (sensor_depth_max - depth) / sensor_depth_max : 0;
            ...
            if(isHaveD2RGBUVMap)
            {
                ...
                if(!getUVWithDXY(depth, i, uvx, uvy))
                {
                    ...
                    if (cx >= 0 && cx < rgb_stream.width && cy >= 0 && cy < rgb_stream.height)
                    {
                        ...
                        int cx2 = (int)(uvx * rgb_registered.cols + 0.5f);
                        int cy2 = (int)(uvy * rgb_registered.rows + 0.5f);

                        rgb_registered.at<cv::Vec3b>(cy2,cx2)[0] = b;
                        rgb_registered.at<cv::Vec3b>(cy2,cx2)[1] = g;
                        rgb_registered.at<cv::Vec3b>(cy2,cx2)[2] = r;
...
//                }
    ...
    if(isHaveD2RGBUVMap)
    {
        ...
        //pubRealSenseRGBRegImageMsg(rgb_registered);
    }
...
{
    return realsense_reg_points_pub.getNumSubscribers() + realsense_rgb_image_pub.getNumSubscribers() + realsense_rgb_registered_image_pub.getNumSubscribers();
}
...
int main(int argc, char* argv[])
{
    ...
    private_node_handle_.param("depth_uv_enable_max", depth_uv_enable_max, 1000);
    ...
    private_node_handle_.param("topic_image_rgb_reg_id", topic_image_rgb_reg_id, std::string("/image/rgb_registered"));
    ...
    depth_frame_buffer = new unsigned short[depth_stream.width * depth_stream.height *2];
    ...
    realsense_rgb_registered_image_pub = image_transport.advertiseCamera(topic_image_rgb_reg_id, 1);
    ...
    rgb_camera_info_man = new camera_info_manager::CameraInfoManager(rgb_handle,"realsense_rgb_camera");
    depth_camera_info_man = new camera_info_manager::CameraInfoManager(depth_handle,"realsense_depth_camera");

    rgb_camera_info_man->loadCameraInfo(rgb_camera_info_url);
    depth_camera_info_man->loadCameraInfo(depth_camera_info_url);
