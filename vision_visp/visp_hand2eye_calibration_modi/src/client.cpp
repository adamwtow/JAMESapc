...
Client::Client()
{
	camera_object_publisher_=n_.advertise<geometry_msgs::Transform> (visp_hand2eye_calibration::camera_object_topic, 1,1);
	world_effector_publisher_= n_.advertise<geometry_msgs::Transform> (visp_hand2eye_calibration::world_effector_topic, 1,1);

	//prepare function objects
	camera_object_subscriber_callback_t camera_object_callback_ = boost::bind(&Client::cameraObjectCallback_, this, _1);
	world_effector_subscriber_callback_t world_effector_callback_ = boost::bind(&Client::worldEffectorCallback_, this, _1);
	start_subscriber_callback_t start_callback_ = boost::bind(&Client::startCallback_, this, _1);


	//define subscribers
	camera_object_subscriber_ = n_.subscribe("/baxter_camera_object", 10,camera_object_callback_);
	world_effector_subscriber_ = n_.subscribe("/baxter_world_effector", 10,world_effector_callback_);

	start_subscriber_ = n_.subscribe("/calib_start", 10,start_callback_);

	//test_sub = n_.subscribe("/baxter_world_effector", 100,&Client::testCallback,this);


	reset_service_= n_.serviceClient<visp_hand2eye_calibration::reset> (visp_hand2eye_calibration::reset_service);
	compute_effector_camera_service_= n_.serviceClient<visp_hand2eye_calibration::compute_effector_camera> (visp_hand2eye_calibration::compute_effector_camera_service);
	compute_effector_camera_quick_service_=n_.serviceClient<visp_hand2eye_calibration::compute_effector_camera_quick>(visp_hand2eye_calibration::compute_effector_camera_quick_service);
	start_calibration=0;
	ROS_INFO("Client");
	cMo_vec.clear();
	wMe_vec.clear();
...
void Client::passToCalibration()
{
	ros::Duration(1.).sleep();
	cout<<"size="<<cMo_vec.size()<<endl;
	for (int i=0;i<cMo_vec.size();i++)
	{
		geometry_msgs::Transform pose_c_o;
		pose_c_o = visp_bridge::toGeometryMsgsTransform(cMo_vec[i]);
		geometry_msgs::Transform pose_w_e;
		pose_w_e = visp_bridge::toGeometryMsgsTransform(wMe_vec[i]);
		camera_object_publisher_.publish(pose_c_o);
		world_effector_publisher_.publish(pose_w_e);
		emc_quick_comm.request.camera_object.transforms.push_back(pose_c_o);
		emc_quick_comm.request.world_effector.transforms.push_back(pose_w_e);
...
void Client::cameraObjectCallback_(const geometry_msgs::Transform::ConstPtr& trans)
{
	ROS_INFO("cameraObjectCallback_");
	ROS_INFO("new cMo: [%f,%f,%f -- %f,%f,%f,%f]", trans->translation.x,trans->translation.y,trans->translation.z,trans->rotation.x,trans->rotation.y,trans->rotation.z,trans->rotation.w);
	vpHomogeneousMatrix cMo;
	cMo = visp_bridge::toVispHomogeneousMatrix(*trans);
	cMo_vec.push_back(cMo);
}

void Client::startCallback_(const std_msgs::Empty::ConstPtr& msg)
{
	ROS_INFO("startCallback");
	start_calibration=1;
}


void Client::worldEffectorCallback_(const geometry_msgs::Transform::ConstPtr& trans)
{
	ROS_INFO("worldEffectorCallback_");
	ROS_INFO("new wMe: [%f,%f,%f -- %f,%f,%f,%f]", trans->translation.x,trans->translation.y,trans->translation.z,trans->rotation.x,trans->rotation.y,trans->rotation.z,trans->rotation.w);
	vpHomogeneousMatrix wMe;
	wMe = visp_bridge::toVispHomogeneousMatrix(*trans);
	wMe_vec.push_back(wMe);
}
